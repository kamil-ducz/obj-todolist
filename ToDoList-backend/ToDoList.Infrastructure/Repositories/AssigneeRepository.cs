...
public class AssigneeRepository : IAssigneeRepository
{
    ...
    {
        return _toDoListDbContext.Assignee!.ToList();
    }

    public Assignee GetAssignee(int assigneeId)
    {
        return _toDoListDbContext.Assignee!.First(a => a.Id == assigneeId);
    }

    public void DeleteAssignee(int assigneeId)
    {
        var assigneeToDelete = _toDoListDbContext.Assignee!.First(a => a.Id == assigneeId);
        _toDoListDbContext.Assignee!.Remove(assigneeToDelete);
        _toDoListDbContext.SaveChanges();
    }

    public void InsertAssignee(Assignee assignee)
    {
        _toDoListDbContext.Assignee!.Add(assignee);
        _toDoListDbContext.SaveChanges();
    }

    public void UpdateAssignee(Assignee assigneeToUpdate)
    {
        _toDoListDbContext.Assignee!.Update(assigneeToUpdate);
        _toDoListDbContext.SaveChanges();
    }
}